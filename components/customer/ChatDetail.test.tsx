...
describe('ChatDetail', () => {
  ...
  it('메시지 리스트가 렌더링되어야 함 (T13)', async () => {
    const { ChatDetail } = await import('./ChatDetail');
    render(<ChatDetail sessionId="session-1" />);

    // 메시지 내용 확인
    expect(screen.getByText(/안녕하세요|도와드릴까요/i)).toBeDefined();
  });
  ...
  it('메시지 전송 기능이 렌더링되어야 함 (T13)', async () => {
    ...
    // 메시지 입력 필드 확인
    const inputField = screen.queryByPlaceholderText(/메시지|Message|입력/i);
